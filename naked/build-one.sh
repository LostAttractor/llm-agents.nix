...
set -u
N="$1"
R=/home/zimbatm/llm-agents.nix/naked
...
echo "== smoke run =="
main=$(ls "$out/bin" 2>/dev/null | head -1)
HOME="/tmp/nh-$N" timeout 30 "$out/bin/$main" --version 2>&1 | head -2 || \
  HOME="/tmp/nh-$N" timeout 30 "$out/bin/$main" --help 2>&1 | head -2 || echo "(no --version/--help; built OK)"
